...
export class ChatGateway {
  ...
  async addGroup(
    ...
    const isUser = await this.userRepository.findOneBy({ userId: data.userId })
    if (isUser) {
  ...
  async joinGroup(
    ...
    if (isUser) {
      ...
      let userGroup = await this.groupUserRepository.findOneBy({
  ...
  async sendGroupMessage(@MessageBody() data: GroupMessageDto): Promise<any> {
    const isUser = await this.userRepository.findOneBy({ userId: data.userId })
    if (isUser) {
      const userGroupMap = await this.groupUserRepository.findOneBy({
        userId: data.userId,
        groupId: data.groupId
      })
  ...
  async addFriend(
    ...
    if (isUser) {
      if (data.friendId && data.userId) {
        ...
        const relation1 = await this.friendRepository.findOneBy({
          userId: data.userId,
          friendId: data.friendId
        })
        const relation2 = await this.friendRepository.findOneBy({
  ...
  async joinFriend(
    ...
    if (data.friendId && data.userId) {
      const relation = await this.friendRepository.findOneBy({
        userId: data.userId,
        friendId: data.friendId
      })
      const roomId =
  ...
  async getAllData(
    ...
    if (isUser) {
      ...
      const groupPromise = groupMap.map(async (item) => {
        return await this.groupRepository.findOneBy({ groupId: item.groupId })
      })
      const groupMessagePromise = groupMap.map(async (item) => {
        ...
        for (const message of groupMessage) {
          if (!userGather[message.userId]) {
            userGather[message.userId] = await this.userRepository.findOneBy({
  ...
  async exitGroup(
    ...
    const map = await this.groupUserRepository.findOneBy({
      userId: groupMap.userId,
      groupId: groupMap.groupId
    })
    if (user && group && map) {
  ...
  async exitFriend(
    ...
    const user = await this.userRepository.findOneBy({ userId: userMap.userId })
    const friend = await this.userRepository.findOneBy({
      userId: userMap.friendId
    })
    const map1 = await this.friendRepository.findOneBy({
      userId: userMap.userId,
      friendId: userMap.friendId
    })
    const map2 = await this.friendRepository.findOneBy({
      userId: userMap.friendId,
